...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ isset($news) ? route('backend.news.update', $news->id) : route('backend.news.store') }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date" class="form-control" id="date"
                        value="{{ old('date', isset($news) && $news->date ? \Carbon\Carbon::parse($news->date)->format('Y-m-d') : '') }}"
                        </div>
                    ...
                    <a href="{{ route('backend.news.index') }}" class="btn btn-secondary">Cancel</a>
            </form>
        </div>
    </div>
@endsection
