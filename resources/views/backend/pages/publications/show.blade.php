            ...
            <div>
                @if($publications->file && file_exists(storage_path('app/public/'.$publications->file)))
                    <div class="d-flex gap-2">
                        <a href="{{ route('backend.publications.open', $publications->id) }}" target="_blank" class="btn btn-secondary">
                            <i class="bi bi-eye"></i> Open File
                        </a>
                        <a href="{{ route('backend.publications.download', $publications->id) }}" class="btn btn-custom">
